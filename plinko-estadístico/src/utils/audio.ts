class SoundManager {
  ...
  public playPegHit(level: number) {
    ...
    try {
      ...
      // Pitch pitch slightly higher for lower levels
      const baseFreq = 400 + level * 35 + (Math.random() * 40 - 20);
      osc.type = 'sine';
  ...
  public playBinLanding(binIndex: number) {
    if (!this.isEnabled) return;
    this.initContext();
    if (!this.ctx) return;
