...
export default function App() {
  ...
  // Launch handler ("Lanzar 1 pelota", "Lanzar 50 pelotas", etc.)
  ...
  const handleBatchFinished = useCallback(() => {
    ...
    // Trigger confetti celebration for 50 or more balls
    try {
      confetti({
        particleCount: 40,
        spread: 60,
  ...
  return (
    <div className="min-h-screen bg-slate-100 font-sans text-slate-800 antialiased selection:bg-purple-200 selection:text-purple-900 pb-12">
      {/* Header Bar */}
      <Header
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 pt-4 sm:pt-6">
        ...
        <div
          className={`transition-all duration-300 mx-auto ${
            isMobileFrame
              ? 'max-w-[420px] bg-slate-950 p-3 rounded-[40px] shadow-2xl border-4 border-slate-800 ring-1 ring-slate-700/50 my-2'
              : 'w-full'
          }`}
        >
          {/* Smartphone Status Bar (When in mobile frame view) */}
          {isMobileFrame && (
            <div className="flex items-center justify-between px-4 py-1.5 text-[11px] font-bold text-slate-400 mb-2">
              <span>09:41</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
              ...
              <div className="lg:col-span-7 space-y-3">
                <div className="flex items-center justify-between px-1">
                  <h2 className="text-sm font-extrabold text-slate-700 uppercase tracking-wider flex items-center space-x-2">
                    <span className="w-2 h-2 rounded-full bg-blue-600 animate-ping inline-block" />
                    <span>Tablero Plinko (10 Niveles)</span>
                  </h2>
                  <span className="text-xs font-semibold text-purple-700 bg-purple-100 px-2.5 py-0.5 rounded-full">
                    Galton Board B(10, {probabilityP})
                  </span>
                </div>
