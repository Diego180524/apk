...
export function generateInterpretation(
  ...
  let summary = '';
  if (n <= 10) {
    summary = `Simulación de n = ${n} pelotas en la cuadrícula rectangular de 10 niveles. Con muestras pequeñas, la variabilidad aleatoria domina cada trayectoria, provocando una distribución heterogénea entre los compartimentos de la base.`;
  } else if (n <= 25) {
    summary = `Simulación de n = ${n} pelotas descendiendo por la estructura en cuadrícula. Se observa una tendencia clara hacia la estabilización de frecuencias en los compartimentos centrales (Cajas 4, 5 y 6), reduciendo el efecto de las fluctuaciones aleatorias extremas.`;
  } else {
    summary = `Simulación acumulada de n = ${n} pelotas en la cuadrícula de obstáculos. Con esta muestra de mayor tamaño, la distribución empírica converge con notable precisión hacia una curva acampanada simétrica centrada en la Caja 5.`;
  }
  ...
  // Media
  details.push(`Media Muestral (x̄ = ${mean.toFixed(2)}): Indica la posición promedio del centro de gravedad de la distribución en la base del tablero (valor teórico central = 5.0).`);

  // Mediana
  details.push(`Mediana (Me = ${median.toFixed(2)}): Divide la muestra en dos partes iguales (50% de las pelotas cayeron a la izquierda o en este valor). ${Math.abs(mean - median) <= 0.25 ? 'La coincidencia cercana entre media y mediana demuestra simetría en el experimento.' : 'La leve diferencia evidencia un sesgo muestral momentáneo.'}`);

  // Moda
  if (mode.length > 0) {
    const modeStr = mode.map((m) => `Caja ${m}`).join(', ');
    details.push(`Moda (Mo = ${modeStr}): Representa el compartimento o compartimentos donde se registró el mayor número absoluto de aterrizajes.`);
  } else {
    details.push('Moda: No se identifica una única caja predominante debido a la dispersión de los datos.');
  }

  // Dispersión y Variabilidad
  details.push(`Dispersión de los Resultados: La desviación estándar muestral es s = ${stdDevSample.toFixed(2)} (varianza s² = ${stats.varianceSample.toFixed(2)}) con un Coeficiente de Variación CV = ${cv.toFixed(1)}%, reflejando una variabilidad ${cv < 30 ? 'baja' : cv < 50 ? 'moderada' : 'alta'}.`);

  // Concentración de Resultados & Rango
  details.push(`Amplitud y Rango: Las pelotas cubrieron un rango de R = ${range} compartimentos, desde el límite inferior (Caja ${min}) hasta el límite superior (Caja ${max}).`);

  // Simetría y Sesgo
  if (shapeClassification === 'Simétrica') {
    details.push(`Simetría Observada: El coeficiente de asimetría de Pearson (${skewness.toFixed(2)}) confirma un comportamiento equilibrado hacia ambos lados de la cuadrícula.`);
  } else if (shapeClassification === 'Sesgada a la derecha') {
    details.push(`Sesgo Positivo (A la derecha): Muestra mayor concentración acumulada en las cajas 0 a 4, extendiéndose una cola menor hacia la derecha.`);
  } else {
    details.push(`Sesgo Negativo (A la izquierda): Muestra mayor concentración acumulada en las cajas 6 a 10, extendiéndose una cola menor hacia la izquierda.`);
  }
