export interface StatisticalInterpretation {
  summary: string;
  details: string[];
  shapeClassification: 'Simétrica' | 'Sesgada a la derecha' | 'Sesgada a la izquierda';
  dispersionClassification: 'Poco dispersa' | 'Dispersión moderada' | 'Muy dispersa';
  rangeExplanation: string;
  outlierExplanation: string;
  learnedConclusion: string; // 3-5 lines educational synthesis
  trivia: {
    title: string;
    fact: string;
    category: string;
